...
## ChatGPT 专区：Codex Desktop 外观

新增 [`chatgpt-codex.user.js`](./chatgpt-codex.user.js) 和 [`chatgpt/extension`](./chatgpt/extension/)：

**插件 ZIP 直接下载：** [`chatgpt-codex-desktop-extension.zip`](https://github.com/frankichen/linuxdo-idea-ui/releases/latest/download/chatgpt-codex-desktop-extension.zip)

下载后解压，在 Chrome / Edge 的扩展管理页开启「开发者模式」，选择「加载已解压的扩展程序」并指向解压目录。

- 油猴脚本：把 `chatgpt.com` 换成 Codex Desktop 风格，保留 ChatGPT 原生数据和交互。
- Chrome / Edge 扩展：普通 ChatGPT 标签页不注入任何壳层；点击扩展图标后，把当前 ChatGPT 标签页移动到无普通地址栏/标签栏的 popup 独立窗口。
- 鼠标触碰窗口最顶部 3px，临时滑出地址栏 + 浏览器书签；离开后自动收起。
- `Alt + Shift + B` 可随时显示 / 隐藏临时工具栏。
- 扩展只读取本机书签用于本地展示，不代理 ChatGPT 请求。

安装和开发说明见 [`chatgpt/README.md`](./chatgpt/README.md)。
